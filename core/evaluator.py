...
class Evaluator:
    ...
    @staticmethod
    def evaluate_kpi(value: float, threshold: float, compare: str = ">=") -> str:
        """
    ...
    def evaluate_all(kpi_values: KPIValues) -> Tuple[KPIStatus, str]:
        ...
        status = KPIStatus(
            sgnb_add_success_rate=Evaluator.evaluate_kpi(
                kpi_values.sgnb_add_success_rate, thresh.SGNB_ADD_SUCCESS_RATE_MIN, ">="
            ),
            sgnb_drop_rate=Evaluator.evaluate_kpi(
                kpi_values.sgnb_drop_rate, thresh.SGNB_DROP_RATE_MAX, "<="
            ),
            dl_user_throughput=Evaluator.evaluate_kpi(
                kpi_values.dl_user_throughput, thresh.DL_THROUGHPUT_MIN, ">="
            ),
            ul_user_throughput=Evaluator.evaluate_kpi(
                kpi_values.ul_user_throughput, thresh.UL_THROUGHPUT_MIN, ">="
            ),
